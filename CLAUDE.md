...
- **Config values are calibrated, not nominal**: `wheel_diameter_m` (~0.0463, effective ≠ nominal 48 mm) and `center_to_wheel_m` (~0.0460) came from `scripts/calibrate.py`; `gyro_scale_z` (~0.984) came from the camera check below. Don't "fix" them back to spec values, and don't hard-code them in tests — read them off `RobotConfig` (see `tests/test_smoke.py`).

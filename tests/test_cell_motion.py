...
def test_the_machine_never_stops_at_a_corner(motion):
    ...
    # (RampLimits) では届かない**。谷は設定で変わる: ランプ 0.9 / decel 0.8 では
    # 0.640 x v_max、**#107 の 0.7 / 0.6 では 0.454** と深くなった。
    # 理由は谷の形ではなく**ブレンドがセルに対して大きくなった**こと — 75mm は 180mm の
    # 42% で、この経路 (3 区間 = 2 コーナー) では中央の区間の入りと出のブレンドが
    # ほぼ重なり、新しい軸が v_max に達し切らない。実走の `--chain-legs 2` は
    # 1 チャンク 1 コーナーなのでこれは起きない。ランプを 5.0 まで緩めても 0.609 止まり
    # なので、谷を決めているのはランプよりこの重なりの方。
    assert min(cruise) > 0.40 * motion.cfg.v_max

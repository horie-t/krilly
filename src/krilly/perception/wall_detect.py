...
#: **校正済みの解像度は 1 つだけにしてある。** 以前は 640x480 (センサーの 33% しか
#: 使わない従来モード) の表も持っていたが、機体を組み直してカメラの高さが変わった
#: 時点で**その表は実機と合わなくなった**。使っていない解像度の古い校正を残すのは、
#: #56 / #88 が繰り返し痛い目を見た「ROI が帯から外れたまま走る」ものそのもので、
#: ``geometry_for`` が未知の解像度で例外を出す方が安い。必要になったら
#: ``camera_fov --emit-bands`` で測って足すこと。
CALIBRATED_BANDS_BY_SIZE = {
    # 960x720 全画素モード (#88)。画角 1.5 倍・px/mm は据え置き 1.70。
    # **機体を組み直したら測り直すこと** — 床板の撓み対策でバッテリ位置を下げた
    # 改修後、カメラが約 0.5% 低くなり (px/mm 1.694->1.706)、帯が 4-5px 動いた。
    # 赤割合には効かない量だが、ROI の中心は位置測定のゼロ点なので 3mm のバイアスになる。
    (960, 720): {
        FRONT: (195, 218),
        BACK: (501, 525),
        LEFT: (315, 337),

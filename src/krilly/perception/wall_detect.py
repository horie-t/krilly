...
#
# #78: 淡く写る壁が 140 の縁からこぼれていたので 125 へ下げる。8x8 の探索ラン
# 1 回 (21 フレーム / 204 ラベル / 壁の帯 90 本) を後からラベル付けして測った。
#
# **帯の彩度は 70 から 213 までの連続分布で、色相はそれに連動する** (S 70 → H 131、
# S 198 → H 169)。最大の隙間が 12 しかないので「2 つのロット」には割れない —
# 淡く写るほど色相がマゼンタ側 (H の小さい方) へ寄る、という 1 本の傾向である。
# h2_lo=140 で読みが弱かった帯ほど、その 140 より下に画素を持っていた:
#   赤割合 0.092 → 帯 H131 S 70、画素の 81% が H<140
#   赤割合 0.176 → 帯 H138 S 79、53%
#   赤割合 0.279-0.373 → 帯 H142-144 S 91-94、33-35%
#   赤割合 0.84-0.88 (最強) → 帯 H166-170 S185-198、0-6%
# 強い帯でも 4-20% は 140 の下にあるので、切り落としは連続的で、閾値をまたいだ帯が
# 弱く写っていただけ。left の一番弱い壁は 0.092 = しきい値 0.08 の **1.1 倍**しか
# 余裕が無く、1 フレームの揺らぎで見落としになる距離だった。
#
# 下限の決め方 (`wall_detect --batch` で h2_lo を振り、壁側と壁なし側を同時に見る):
#   h2_lo   140    135    130    125    120    115
#   壁 最小 0.092  0.201  0.352  0.443  0.447  0.447   ← 125 で飽和
#   開 最大 0.003  0.003  0.003  0.003  0.003  0.010   ← 115 で漏れ始める
# 壁側だけが上がるので、落ちていたのは本物の壁。漏れの正体は**機体の青配線**で、
# その色相は H 100-115 に 63% が集中し、120-140 には 1% しかない (FRONT/LEFT の
# ROI は機体が写る領域と重なるので他人事ではない)。125 は飽和点であると同時に
# 漏れ点まで 10 の余裕がある。**これ以上下げても得るものは無く、失うものだけがある。**
#
# 測り方の注意: **色相の中央値をマスクが通した画素から取ってはいけない。**
# それは h2_lo で下から切られた分布なので、どんなに淡い帯でも「H は 140 以上」と
...
CALIBRATED_BANDS_BY_SIZE = {
    # 640x480 (センサーの 33% しか使わない従来モード)。傾き調整後に再測定し、
    # 2026-09-24 のカメラの付け直しの後にも 960x720 と同じ姿勢で測り直した
    # (こちらも px/mm 1.70 -> 1.67)。実機では使っていないが、ROI の形が解像度によらず
    # 同じであることの検算に使う。
    (640, 480): {
        FRONT: (91, 114),
        BACK: (393, 414),
    ...
    # 960x720 全画素モード (#88)。画角 1.5 倍。
    # 2026-09-24 に再測定: ボタン・ブザー (#79) の取り付けでカメラを外して付け直したら、
    # ケーブルの張りが緩んで天板の沈み込みが減り、**カメラが 5-7mm 高くなった**
    # (px/mm X 1.706->1.672 / Y 1.697->1.675、帯は前 +13px / 後 +9px / 左 +8px)。
    # 壁の有無は読めていた (探索 ±40px の内) が、向かい合う壁の位置の読みが前後 4.9mm /
    # 左右 6.3mm 食い違い、位置補正が機体を中央からずらす状態だった。黒い床・EV -2・
    # 定規でセル中央に置いて 3 回測り、1px 以内で一致。傾きは計 0.7° で変化なし。
    (960, 720): {
        FRONT: (212, 233),
        BACK: (514, 534),
        LEFT: (318, 339),

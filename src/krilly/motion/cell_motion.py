...
class CellMotionConfig:
    ...
    # 主軸の速度上限と減速エンベロープ (#21 の実機ラダー + #103 の通しラン)。
    # 直進: v=0.12/0.18/0.24/0.30 を 4 セル直進で比較。所要は理論値どおり縮み
    # (6.25/4.27/3.29/2.69s)、距離誤差は v>=0.24 で 720mm あたり平均 -5mm・幅 ±4mm、
    # v<=0.18 では ±2mm。
    # **#21 は「0.30 は 4 セルで 0.5s しか稼げない」として 0.24 を採ったが、それは
    # 尺度の取り違えだった** (#87 が指摘し #103 が実測): 1 動作なら誤差でも、7 分の
    # セッションでは大会迷路 30 面のうち**最速ランを 1 本も走れない面が 3 面から
    # 0 面**になり、最速ランの総数が 38 本から 54 本へ増える (8x8 の実測定数で
    # シミュレート、chain-legs 2)。実機の 8x8 通しランでも探索 33.2->31.2s /
    # 最速 27.6->25.6s で、L6470 のフォールトゼロ・中断ゼロ・学習した地図も同じ。
    # 横移動の精度も落ちない (#87 の姿勢を揃えた 4 セル横移動の対照)。
    # **0.30 は MAX_SPEED 500 と対で使う**: 横移動の指令ピークが 453 step/s なので、
    # 400 のままだと W0 が頭打ちになり 3 輪の速度比が崩れて経路が曲がる。
    # ``tuning.check_limits`` がこの組み合わせを警告する — **上げる前に必ず通すこと。**

...
class RampLimits:
    """加減速の上限 (#21 の実機ラダー、並進は #107 で 0.9 -> 0.6 へ)。
    ...
    **並進が 0.7 なのはコーナーのため** (#107)。``vx``/``vy`` は独立に律速されるので、
    止まらずに曲がる間 (#80) は両軸が同時に変化し、体加速度は
    ``sqrt(accel^2 + decel^2)`` になる。**コーナーで減速はしていない** — 両軸とも
    ``v_max`` まで許され、入口と出口は 100% の ``v_max``、中央で 76% まで落ちるのは
    入れ替わりの副産物。上がるのは加速度だけで、しかも **v_max には依存しない**。
    0.9/0.8 では 1.20 m/s^2 で、**5 回に 1 回、コーナーの出口で機体が回って中断**して
    いた (方位残差がガードの 2.5° を超える。8x8 の 56 動作に 2.37° が 1 件)。
    0.7/0.6 は 0.92 m/s^2 で、**コーナー付き動作 56 件の最大が 1.08°、1.5° 超はゼロ**。
    0.6/0.6 (0.85 m/s^2) も 280 動作で最大 1.05° と同等だが、**そちらは採れない** —
    次の段落を見ること。代償は最速ラン +4% (19.3 -> 20.0s) と、ブレンド距離が
    56 -> 75mm に伸びて内側の柱の余裕が 25.9 -> 21.9mm になること (廊下は 21.4mm)。

    **``decel`` は ``accel`` より小さくなければならない。等号は駄目** (#107)。
    減速エンベロープ ``sqrt(2*decel*r)`` は**ちょうど ``decel`` の減速を要求する**ので、
    ランプが同値だと追従の余裕がゼロになり、許容帯を越えて coast する。実測:
    0.6/0.6 (余裕 0%) は 1 セル移動の行き過ぎが **+3.00mm** (実機 140 動作、範囲 ±3.10)
    で ``retry_pos_tol_m`` 4.0mm のすぐ下に居座り、0.7/0.6 (余裕 14%) は **+0.20mm**
    (範囲 -0.20〜+0.30)。従来の 0.9/0.8 も余裕 11% で +0.36mm だった。
    ``check_limits`` は「decel > accel」だけを見て等号を許すので、**この罠は自動では
    防げない**。余裕は 10% 以上取ること。
    """

    max_linear_accel_mps2: float = 0.7      # vx, vy の加速度上限 [m/s^2] (#107)

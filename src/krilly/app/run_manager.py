...
class RunManager:
    ...
    # #21 の採用点 (v=0.24 m/s, accel 0.9, omega=2.0 rad/s) での値。
    #   直進: 4 セル 1 動作を実測 3.29s。台形プロファイルの式に当てると
    #         1 セルあたり 0.75s + 1 回あたり 0.28s (連続直進はランプの固定費が
    #         償却されるので、固定費を分離しないと直進の多い経路で見積もりが外れる)
    #   旋回: 90° を実測 1.56s (4 回の平均)
    #   これに動作間の停止 (--pause) が 1 回あたり +0.44s 乗る。
    # **速度設定を変えたら speed_run の末尾に出る実測表で測り直すこと。**
    cell_time_s: float = 0.75          # 直進の 1 セルあたり増分
    straight_time_s: float = 0.72      # 直進 1 回あたりの固定費 (ランプ + 整定 + 停止)
    turn_time_s: float = 2.00          # 90° 旋回 1 回 (整定 + 停止込み)

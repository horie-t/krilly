...
class RunManager:
    ...
    #   動作あたりの固定費 **0.99s** (#107 で並進ランプを 0.9 -> 0.7 / decel 0.8 -> 0.6
    #   に下げたぶん伸びた。8x8 実測: 最速ラン 20 セル / 15 区間 -> 8 動作を 20.00s、
    #   (20.00 - 12.10) / 8 = 0.99s で、見積もりは実測に +0.1%)。
    #   **セル時間は変わらない** — 84 動作の最小二乗でも南北 0.586 / 東西 0.590 で、
    #   伸びたのは固定費だけ 0.43 -> 0.54s。ランプ超過 v/2*(1/accel+1/decel) が
    #   0.354 -> 0.464s になるのが主因。accel 0.9 / decel 0.8 では 0.90s だった (#103)
    ...
    straight_time_s: float = 0.99      # 動作 1 回あたりの固定費 (ランプ + 整定 + 撮影 + 停止)

...
class RunManager:
    ...
    #   南北 (機体の前後軸) 1セル 0.74s + 区間 0.39s
    #   東西 (機体の左右軸) 1セル 0.76s + 区間 0.35s
    #   動作あたりの停止 0.44s (最速ラン実測 25.0s - 動作時間の合計 15.0s を 12 動作で割る)
    # 値は #80 の対照セッション (同じ日・同じ電池で chain-legs 1 と 2 を連続して 5 走ずつ)
    # で取り直したもの。**固定費が動作あたりであること**もこの対で確かめている:
    ...
    cell_time_s: float = 0.74          # 南北 1 セルあたり増分
    straight_time_s: float = 0.83      # 動作 1 回あたりの固定費 (ランプ + 整定 + 撮影 + 停止)
    ...
    lateral_cell_time_s: float = 0.76

...
- `wheel_angles_deg: [90.0, 210.0, 330.0]` ＝ [θ0, θ1, θ2]。
- 3輪とも正回転（正の速度）で接地点が CCW 接線方向に進む → 車体は **+ω（左旋回）**。

## 3. 逆運動学プレビュー（M2 で実装）

ボディ速度 `(vx, vy, ω)` から各ホイールの周速 `v_i`：
...
- 例：純前進 `vx>0, vy=0, ω=0` → `v0=-vx, v1=+0.5vx, v2=+0.5vx`（前輪が逆回転、後2輪が押す）。
- 例：純旋回 `vx=vy=0, ω>0` → `v0=v1=v2=L·ω`（3輪同速正回転で左旋回）。

周速 → ステッパ周波数の変換は `kinematics/kiwi.py`（M2）で
`f_step = 200·μ·v_i/(π·D)`（D=48mm）を用いる。
...
- **前方の定義**：本書は `config/robot.yaml` の `wheel_angles_deg=[90,210,330]` から
  「M0 を正面・M1/M2 を後方」の配置を導いている。実機の組み立てがこれと異なる場合は
  番号割当か config を合わせる。
